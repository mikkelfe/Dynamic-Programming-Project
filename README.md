...
borrowing, and risk-free savings under uncertainty regarding farming returns, farmland 
prices, and financial market returns. We implement the original model in
Python and reproduce the main policy functions and simulation results from the
original study, including optimal farmland investment decisions, wealth accumulation, 
and probabilities of exiting farming.
...
investment decisions, while transaction costs generate inaction in farmland adjustment. 
When transaction costs are removed, land trading becomes more active and
