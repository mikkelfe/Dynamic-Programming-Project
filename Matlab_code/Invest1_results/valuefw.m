...
%VALFUNCW: value function for one control, 4 number of states
%s is set of all states
%x is (nn by 1), c is (nn by 1), s is (nn by 4), t is scalar

global basis n sminv smaxv smin smax e w T rp rn tcs tcb fme fds fdb si
...
if (nargin==3)
   K=1; wk=1;
elseif ((s(:,3)+x)<1)       % this is for g(:,3)==0 or <1
   K=1;
end
...
for k = 1:K
   ...
   if g(:,3)<1                            % if g(:,3)==0,sell all land
      gA = g(:,4);								% g(:,4) is all liquid asset since gL=0
      r = ones(nn,1).*rp;
      ngi = find(gA<0);  						%Negative gA index
      r(ngi) = rn;
      vval = utility((((1+r).^(T-t)).*gA));
   else						%(2)
   	if isempty(c)		%(2.1)
