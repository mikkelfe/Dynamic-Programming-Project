...
/* The following line was changed to ensure 8 byte boundaries for the output array 
   This can be altered when MEX functions ensure this. Also the last lines
   copying z into the output mxArray should be deleted and freeing z.            */
/*   plhs[0]=mxCreateDoubleMatrix(Brows,cn,mxREAL);*/ /* mxArray for output */
/*   z=mxGetPr(plhs[0]);    */                        /* pointer to output data */
   z=calloc(Brows*cn,sizeof(double));
   d=calloc(Brows*Bn,sizeof(double));
   /* ********************************************************************* */
   ...
   /* ******************************************** */
   free(d);
   plhs[0]=mxCreateDoubleMatrix(Brows,cn,mxREAL);
   memcpy(mxGetPr(plhs[0]),z,Brows*cn*sizeof(double));
   free(z); 
}
