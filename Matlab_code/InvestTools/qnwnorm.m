...
% QNWNORM Computes Gauss-Hermite nodes and weights multivariate normal distribution
%
% Usage
%   [x,w] = qnwnorm(n,mu,var)
% Input
%   n   : d by 1 vector of number of nodes for each variable
%   mu  : d by 1 mean vector
...
w = ckron(w);
x = cgrid(x);
x = x*chol(var)+mu(:,ones(1,prod(n),1))';


function [x,w] = qnwnorm1(n);

% QNWNORM1 Computes Gauss-Hermite nodes and weights univariate standard normal distribution
%
% Usage
%    [x,w] = qnwnorm1(n)
% Input
...
for i=1:m
   ...
   if its>=maxit
      error('failure to converge in gqnnorm1')
   end
   x(n+1-i) = z;
   x(i) = -z;
   w(i) = 2./(pp.*pp);
